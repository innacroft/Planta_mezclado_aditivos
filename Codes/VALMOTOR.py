...
titulo = tkFont.Font(family = 'Helvetica', size = 20, weight='bold' )
titulo1 = tkFont.Font(family = 'Helvetica', size = 20)
...
#CALCULO DE ALTURA DE ALTURA DE NIVEL
       ...
       #TANQUE ADITIVO
       global hb
       ha= -0.0203*(Va*1000+41)+43.93   #funcion que describe hac vs vaprint"hac",Decimal (ha)
       global ha
       print"ha", ha
       LB["text"] = Vb
       MA["text"] = Va
...
Bal= Spinbox(win,font= titulo1, from_=45,to=300,borderwidth=1)  #BASELITROS
Bal.grid(row=2,column=2)


Tad= Label(win, text= "Aditivo(ml)",font= titulo1,borderwidth=1 )
Tad.grid(row=3,column=1)
Adm= Spinbox(win,font= titulo1,from_= 1,to=2000, borderwidth=1)#ADITIVOMILILITROS
...
vertir= Button(win, text= "2.Vertir Aditivo ", font= texto, relief= RAISED,command = vertir,height=2 , width= 12).grid(row=8,column=2)
